...
const decodeData = data =>
  data.length > 0 ? JSON.parse(Buffer.from(data).toString()) : null
...
class AbstractRequestSwarm extends AbstractSwarm {
  ...
  _handleConnection(socket) {
    ...
    const createRequest = this._createRequestMethod(stream)
  ...
  _createRequestMethod(stream) {
    return (method, path, data) =>
      new Promise((resolve, reject) => {
        ...
        const goodCode = requestTypeMapping[reqMethod].toLowerCase()
